...
class OrthogonalTokenAdapter(nn.Module):
    """
    Maps each expert hidden tensor to a shared compact token space (B, K, D).

    The first stage projects the expert-specific feature dimension to d_model.
    The second stage compresses or remaps the token dimension to token_count.
    """
...
class AttentionResidualBlock(nn.Module):
    """Cross-attends learned forecast tokens to all compact expert tokens."""
...
class AttentionForecastFusion(nn.Module):
    """
    Uses compact expert tokens to directly predict the forecast horizon.
    """
    ...
    def forward(self, tokens_by_expert):
        ...
        for index, name in enumerate(self.expert_names):
            ...
            context_parts.append(tokens + self.expert_embedding[index])
...
class OrthogonalAttentionExpertHeadFusion(nn.Module):
    """
    V4 fusion model.

    Main path:
        hidden_i -> adapter_i -> compact tokens_i
        compact tokens -> learned-query attention -> y_final

    Auxiliary objective:
        L = L_pred(y_final, y)
            + orth_loss_weight * L_orth(compact tokens)
    """
    ...
    def __init__(
        ...
        for name in self.expert_names:
            self.adapters[name] = OrthogonalTokenAdapter(
        ...
        self.attention_fusion = AttentionForecastFusion(
    ...
    def _resolve_expert_dims(self, expert_dims):
        ...
        if missing:
            raise ValueError(
                ...
                + ", ".join(missing)
                + ". OrthogonalAttentionExpertHeadFusion needs each expert hidden dimension."
            )
        return resolved
    ...
    def orthogonal_loss(self, tokens_by_expert):
        ...
        for left, right in itertools.combinations(self.expert_names, 2):
            ...
            left_centered = left_tokens - left_tokens.mean(dim=0, keepdim=True)
            right_centered = right_tokens - right_tokens.mean(dim=0, keepdim=True)
            left_norm = F.normalize(left_centered, dim=-1, eps=1e-6)
            right_norm = F.normalize(right_centered, dim=-1, eps=1e-6)
    ...
    def forward(self, batch_tensor, batch=None, flag="test", return_info=False):
        ...
        tokens_by_expert = {}
        for name in self.expert_names:
            compact_tokens = self.adapters[name](batch_tensor[name])
            tokens_by_expert[name] = compact_tokens

        output_norm, fusion_info = self.attention_fusion(tokens_by_expert)
...
FusionModel = OrthogonalAttentionExpertHeadFusion
